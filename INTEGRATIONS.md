...
**deidproof** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `deidproof` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
deidproof ... --format json | cognis-connect emit --to sigma   # Sigma rules
deidproof ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
deidproof ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
deidproof ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `deidproof` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="deidproof") for rec in records]   # records = your JSON output
